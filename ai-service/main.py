...
class GenerateResponse(BaseModel):
    ...
    # Structured output fields (used by UI tabs)
    query           : str = ""          # alias of generated_query
    schema          : str = ""          # tables + columns used
    deepmap         : str = ""          # join relationships
...
async def generate(req: GenerateRequest):
    ...
    schema_summary = schema_text[:800] if schema_text else ""

    return GenerateResponse(
        generated_query  = cleaned,
        query            = cleaned,              # UI tabs read .query
        model_used       = model_used,
        display_name     = MODELS[model_used]["display_name"],
        explanation      = explanation_text,
        schema           = schema_summary,       # UI Schema tab
        deepmap          = deepmap_text,          # UI Deepmap tab
        confidence_score = confidence_from_query(cleaned),
        tokens_used      = tokens,
        execution_ms     = round(ms, 1),
        fallback_used    = fb,
        tables_used      = tables_used,
        schema_used      = schema_summary,
    )
